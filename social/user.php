...
function callUtopiaAPI($method, $params = []) {
    $data = ['method' => $method, 'token' => $_SESSION['token'] ?? null]; if (!empty($params)) $data['params'] = $params;
...
if(empty($tFullPk) || empty($tHashedPk) || $resolvedNick === 'Unknown User') {
    $cMsgs = callUtopiaAPI('getChannelMessages', ['channelid'=>CHANNEL_ID]);
    if(isset($cMsgs['result']) && is_array($cMsgs['result'])) {
        foreach($cMsgs['result'] as $m) {
            $mPk = $m['pk'] ?? ($m['metaData']['data']['hexPublicKey'] ?? '');
            $mHash = $m['hashedPk'] ?? '';
            if( (!empty($tFullPk) && $mPk === $tFullPk) || (!empty($tHashedPk) && $mHash === $tHashedPk) ) { 
                if(empty($tFullPk) && !empty($mPk) && strlen($mPk) === 64) $tFullPk = $mPk;
                if(empty($tHashedPk) && !empty($mHash)) $tHashedPk = $mHash;
                if($resolvedNick === 'Unknown User' && !empty($m['nick'])) $resolvedNick = $m['nick'];
            }
        }
    }
}

$globalDmUrl = !empty($tFullPk) ? "global_dm.php?pk=".urlencode($tFullPk) : "#";
$globalDmClass = !empty($tFullPk) ? "btn-dm btn-dm-global" : "btn-dm btn-dm-global disabled";
$globalDmClick = empty($tFullPk) ? "onclick=\"alert('Global DM terkunci! Utopia menyembunyikan Full PK orang ini.\\n\\nSilakan gunakan tombol [Channel PM].'); return false;\"" : "";
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'user_posts') {
    ...
    if (!isset($res['result']) || !is_array($res['result'])) { echo json_encode(['messages' => [], 'count' => 0, 'error' => 'API Error']); exit; }
    $msgs = $res['result'];
    
    $uniquePks = [];
    foreach ($msgs as $m) {
        $p = $m['pk'] ?? ''; $h = $m['hashedPk'] ?? '';
        if ((!empty($tFullPk) && $p === $tFullPk) || (!empty($tHashedPk) && $h === $tHashedPk)) {
            if (empty($tFullPk) && !empty($p) && strlen($p) === 64) $tFullPk = $p;
            if (empty($tHashedPk) && !empty($h) && strlen($h) === 32) $tHashedPk = $h;
        }
    ...
    $userMsgs = array_filter($msgs, function($m) use ($targetPk, $tFullPk, $tHashedPk) {
        $p = $m['pk'] ?? ''; $h = $m['hashedPk'] ?? '';
        return ($p === $targetPk) || ($h === $targetPk) || (!empty($tFullPk) && $p === $tFullPk) || (!empty($tHashedPk) && $h === $tHashedPk);
    });
    usort($userMsgs, function($a, $b) { return strtotime($b['dateTime']??0) - strtotime($a['dateTime']??0); });
    
    $detectedName = 'Unknown User';
    if(count($userMsgs) > 0) { $first = reset($userMsgs); $detectedName = $first['nick'] ?? 'Unknown User'; }
    
    $userMsgs = array_slice($userMsgs, $offset, POSTS_PER_PAGE);
    $userAva = $avaCache[$tFullPk] ?? ($avaCache[$tHashedPk] ?? 'data:image/svg+xml;base64,'.base64_encode(generateDefaultAvatar($detectedName)));
    
    foreach ($userMsgs as &$m) {
        $m['displayName'] = $detectedName; $m['avatar'] = $userAva;
...
?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>User Profile - Utopia Social</title>
<style>
...
.btn-dm{padding:8px 16px;background:#00ff41;color:#000;border:none;border-radius:24px;font-weight:700;cursor:pointer;font-size:14px;text-decoration:none;transition:opacity 0.2s;flex:1;text-align:center} .btn-dm:hover{opacity:0.8}
.btn-dm-global{background:#1d9bf0;color:#fff;}
.btn-dm-global.disabled{opacity:0.3; cursor:not-allowed;}
...
<div class="container">
    <div class="header"><div class="header-content"><div class="header-left"><button class="back-btn" onclick="window.location.href='index.php'">←</button><div class="header-title"><div class="header-name" id="topName">Loading...</div><div class="header-count" id="postCount">0 posts</div></div></div></div></div>
    <div class="profile-banner">
        <div class="profile-info">
            <img src="data:image/svg+xml;base64,<?=base64_encode(generateDefaultAvatar('U'))?>" class="profile-avatar" id="profileAvatar">
        ...
        <div class="btn-group">
            ...
            <a href="<?= $globalDmUrl ?>" id="globalDmBtn" class="<?= $globalDmClass ?>" <?= $globalDmClick ?>>Global DM</a>
        </div>
    </div>
...
function loadUserPosts(){
    ...
    fetch('?ajax=user_posts&pk=<?=urlencode($targetPk)?>&offset='+offset).then(r=>r.json()).then(d=>{
        ...
        if(offset===0){ 
            ...
            if(d.resolvedFullPk) {
                let gBtn = document.getElementById('globalDmBtn');
                if(gBtn) {
                    gBtn.href = 'global_dm.php?pk=' + d.resolvedFullPk;
                    gBtn.className = 'btn-dm btn-dm-global';
                    gBtn.onclick = null;
                }
            }
        } else { let ldr=tl.querySelector('.loading'); if(ldr) ldr.remove(); }
        ...
        } else { hasMore=false; tl.insertAdjacentHTML('beforeend', offset===0?'<div class="no-more">This user hasn\'t posted anything yet</div>':'<div class="no-more">No more posts</div>'); }
...
function createPost(m){
    ...
    // Tarik tag HTML gambar dari PHP
...
window.addEventListener('scroll', function(){ if((window.innerHeight+window.scrollY)>=document.body.offsetHeight-500) { if(!loading && hasMore){ document.getElementById('timeline').insertAdjacentHTML('beforeend', '<div class="loading"><div class="loading-spinner"></div></div>'); loadUserPosts(); } } });
loadUserPosts();
</script>
</body></html>