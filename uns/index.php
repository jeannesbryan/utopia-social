...
<head>
    ...
    <title>uNS Manager - Wetopia</title>
...
<div class="main-container">
    ...
    <div class="content-area">
        <div id="tab-my_domains" class="tab-content active">
            ...
            <div class="section-desc">List of your uNS portfolio. Set a primary name for your identity profile!</div>
            ...
            <table id="myUnsTable">
                ...
                <tbody id="myUnsBody"><tr><td colspan="4" class="empty-state">Loading your domains...</td></tr></tbody>
            </table>
        </div>

        <div id="tab-register" class="tab-content">
            <h2 class="section-title">Register New uNS</h2>
            <div class="section-desc">Claim your identity on the decentralized Utopia network. Enter a unique name!</div>
            <div id="regAlert" class="alert-box"></div>
            
            <div class="form-group">
                <label>Desired uNS Name</label>
                <input type="text" id="regName" placeholder="e.g. wetopia">
            </div>
            <div class="form-group">
                <label>Route to Channel ID (Optional)</label>
                <input type="text" id="regChannel" placeholder="Leave blank for personal profile">
            </div>
        ...
        <div id="tab-explorer" class="tab-content">
            ...
            <div class="section-desc">Track ownership, history, and portfolio of a uNS name or Public Key.</div>
            <div class="form-group" style="display: flex; gap: 12px;">
                <input type="text" id="searchQuery" placeholder="Type a uNS name or 64-character PK..." style="flex:1;">
        ...
        <div id="tab-transfer" class="tab-content">
            ...
            <div class="section-desc">Web3 domain handover center. Transfer your domains safely.</div>
...
async function getRegDate(name) {
    ...
    try {
        ...
        if(d.result && d.result.length > 0) {
            ...
            return rawDate ? new Date(rawDate).toLocaleDateString() : 'Unknown';
        }
    } catch(e) {
...
async function loadMyUns() {
    ...
    fetch('?ajax=my_uns').then(r=>r.json()).then(async d => {
        if(d.result && d.result.length > 0) {
            ...
            tbody.innerHTML = '<tr><td colspan="4" class="empty-state">You don\'t own any uNS yet.</td></tr>';
        }
    });
}
...
function togglePrimary(checkbox, name) {
    ...
    .then(d => {
        if(d.result) {
            showAlert('myUnsAlert', 'success', `Change request successfully sent to the blockchain!`);
            ...
            showAlert('myUnsAlert', 'error', 'Failed to update status! Ensure your CRP balance is sufficient.');
            checkbox.checked = !isPrimary;
        }
        checkbox.disabled = false;
    }).catch(() => {
        showAlert('myUnsAlert', 'error', 'Network Error! Failed to contact the Node.');
        checkbox.checked = !isPrimary; 
        checkbox.disabled = false;
    });
}
...
function registerUns() {
    ...
    if(!name) return showAlert('regAlert', 'error', 'Name cannot be empty!');
    ...
    fetch('?ajax=register', {method:'POST', body:fd}).then(r=>r.json()).then(d => {
        if(d.result) {
            showAlert('regAlert', 'success', 'uNS registration sent successfully!');
            document.getElementById('regName').value = '';
            document.getElementById('regChannel').value = '';
            document.getElementById('regPrimary').checked = false;
        } else {
            showAlert('regAlert', 'error', 'Failed: Name taken or insufficient balance.');
        }
...
function searchUns() {
    ...
    resBox.innerHTML = '<div class="empty-state">Scanning blockchain...</div>';
    ...
    fetch('?ajax=search', {method:'POST', body:fd}).then(r=>r.json()).then(async d => {
        if(d.result && d.result.length > 0) {
            ...
            resBox.innerHTML = `<table style="background:#000; border-radius:8px;">
                ...
                <tr><th>Other Domains</th><td id="assocDomains">Scanning portfolio...</td></tr>
            </table>`;
            ...
            fetch('?ajax=search_by_pk', {method:'POST', body:fdPk}).then(rp=>rp.json()).then(dp => {
                ...
                if(dp.result && dp.result.length > 0) {
                    ...
                    assocBox.innerHTML = badges || '<span style="color:#71767b;">No other domains found.</span>';
                } else {
                    assocBox.innerHTML = '<span style="color:#71767b;">No other domains found.</span>';
                }
            });
            ...
            fetch('?ajax=history', {method:'POST', body:fdHist}).then(rh=>rh.json()).then(dh => {
                if(dh.result && dh.result.length > 0) {
                    ...
                    dh.result.forEach(hx => {
                        ...
                        let displayDate = rawHistDate ? new Date(rawHistDate).toLocaleString() : 'Unknown Date';
            ...
            let emptyMsg = query.length === 64 ? 'This PK doesn\'t have any registered uNS yet!' : 'Name is available and ready to be registered!';
...
function transferUns() {
    ...
    if(!name || pk.length !== 64) return showAlert('tfAlert', 'error', 'Enter the name and a 64-character PK!');
    ...
    fetch('?ajax=transfer', {method:'POST', body:fd}).then(r=>r.json()).then(d => {
        if(d.result && d.result.length > 10) showAlert('tfAlert', 'success', 'Transfer request sent!');
        else showAlert('tfAlert', 'error', 'Failed to send transfer request.');
        btn.disabled = false;
    });
}
